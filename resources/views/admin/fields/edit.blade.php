    ...
    <div class="space-y-8">
        ...
        <div class="bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-lg">
            ...
            <form action="{{ route('admin.fields.update', $field->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label for="image" class="block text-slate-400 text-sm font-medium mb-2">Gambar Utama Lapangan</label>
                        @if($field->image)
                        <div class="mb-3">
                            <p class="text-xs text-slate-500 mb-2">Gambar Saat Ini:</p>
                            <img src="{{ asset('storage/' . $field->image) }}" alt="Current Image" class="h-20 w-auto rounded-lg border border-slate-700 object-cover">
                        </div>
                        @endif
                ...
                <div class="mt-8 flex items-center gap-4">
                    <button type="submit" class="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-colors shadow-lg shadow-emerald-600/30">
                        Update Data Utama
                    </button>
                </div>
            </form>
        ...
        <div class="bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-lg">
            ...
            <form action="{{ route('admin.fields.facilities.update', $field->id) }}" method="POST">
                ...
                <div>
                    <label for="facilities" class="block text-slate-400 text-sm font-medium mb-2">Daftar Fasilitas (Pisahkan dengan koma)</label>
                    <textarea name="facilities" id="facilities" rows="4"
                        class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors">@foreach($field->facilities as $f){{ $f->name }}{{ $loop->last ? '' : ', ' }}@endforeach</textarea>
                    <p class="text-xs text-slate-500 mt-2">Contoh: Wifi Gratis, Kamar Mandi Bersih, Area Parkir</p>
                    @error('facilities') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <button type="submit" class="mt-4 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl transition-colors shadow-lg shadow-blue-600/30">
                    Update Fasilitas
                </button>
            </form>
        </div>
        ...
        <div class="bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-lg">
            <h3 class="text-lg font-bold text-white mb-4 border-b border-slate-700 pb-2">Kelola Galeri Foto Tambahan</h3>

            {{-- Form Tambah Galeri Baru (Upload Multi-file) --}}
            <form action="{{ route('admin.fields.gallery.update', $field->id) }}" method="POST" enctype="multipart/form-data" class="mb-8 p-4 border border-slate-700 rounded-xl">
                @csrf
                @method('PATCH')
                ...
                <div class="flex gap-4 items-end">
                    ...
                    <button type="submit" class="bg-emerald-600 text-white px-6 py-2.5 rounded-xl hover:bg-emerald-500 transition-colors">
                        Upload
                    </button>
                </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="relative group aspect-square rounded-xl overflow-hidden border border-slate-700">
                    <img src="{{ asset('storage/' . $gallery->image) }}" class="w-full h-full object-cover transition group-hover:opacity-70">
                    ...
                    <form action="{{ route('admin.fields.gallery.delete', $gallery->id) }}" method="POST"
                        ...
                        @csrf @method('DELETE')
                        <button type="submit" class="bg-red-600 text-white p-1 rounded-full hover:bg-red-700 transition shadow-lg">
