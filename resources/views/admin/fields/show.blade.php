        ...
        <div class="mb-8 flex items-center justify-between">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.fields.edit', $field->id) }}"
                    class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition">
                    Edit Lapangan
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg relative overflow-hidden">
                <div class="relative z-10">
                    ...
                    <h3 class="text-2xl font-bold text-emerald-400 mt-1">Rp {{ number_format($totalRevenue, 0, ',', '.') }}
                    </h3>
                </div>
            ...
            <div class="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg">
                ...
                <h3 class="text-2xl font-bold text-white mt-1">{{ $totalBookings }} <span
                        class="text-sm font-normal text-slate-500">Reservasi</span></h3>
            </div>

            {{-- Card 3: Tarif --}}
            <div class="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg">
                <p class="text-slate-400 text-sm font-medium">Tarif Saat Ini</p>
                <h3 class="text-2xl font-bold text-blue-400 mt-1">Rp {{ number_format($field->hourly_rate, 0, ',', '.') }}
                    <span class="text-sm font-normal text-slate-500">/ jam</span></h3>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-lg">
                    <div class="aspect-video w-full bg-slate-900 relative">
                        ...
                        @endif
                        <div
                            class="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-lg text-white text-xs font-bold uppercase">
                    ...
                    <div class="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach ($field->galleries as $gallery)
                                <div
                                    class="aspect-square rounded-xl overflow-hidden border border-slate-700 cursor-pointer group relative">
                    ...
                    @if ($upcomingBookings->count() > 0)
                        <div class="space-y-4">
                            @foreach ($upcomingBookings as $booking)
                                <div
                                    class="p-4 bg-slate-700/30 rounded-xl border border-slate-700 hover:border-slate-500 transition">
                                    <div class="flex justify-between items-start mb-2">
                                        <span
                                            class="text-emerald-400 text-xs font-bold uppercase tracking-wider bg-emerald-500/10 px-2 py-1 rounded">
                                            {{ \Carbon\Carbon::parse($booking->booking_date)->format('d M') }}
                                        </span>
                                        <span class="text-slate-400 text-xs">{{ $booking->start_time }} -
                                    ...
                                    <div class="flex justify-between items-center mt-2">
                                        ...
                                        <a href="{{ route('admin.reservations.show', $booking->id) }}"
                                            class="text-blue-400 text-xs hover:underline">Detail &rarr;</a>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <div class="mt-6 text-center">
                            <a href="{{ route('admin.reservations.index') }}"
                                class="text-sm text-slate-400 hover:text-white transition">Lihat semua jadwal</a>
                        </div>
                    @else
